export async function seedCategories(prisma: any) {
  ...
  // ======================
  const personalTransport = await prisma.category.create({
    data: {
      name: 'Персональный транспорт',
      slug: 'personal-transport',
      parentId: transportTourism.id,
    },
  });

  await prisma.category.createMany({
    data: [
      { name: 'Самокаты', slug: 'scooters', parentId: personalTransport.id },
      {
        name: 'Электрические самокаты',
        slug: 'electric-scooters',
        parentId: personalTransport.id,
      },
      { name: 'Мото байк', slug: 'motorbikes', parentId: personalTransport.id },
    ],
  });
  ...
  await prisma.category.createMany({
    data: [
      {
        name: 'Электро велосипеды',
        slug: 'electric-bicycles',
        parentId: bicycles.id,
      },
      {
        name: 'Классические велосипеды',
        slug: 'classic-bicycles',
        parentId: bicycles.id,
      },
      {
        name: 'Трициклы для детей',
        slug: 'kids-tricycles',
        parentId: bicycles.id,
      },
    ],
  ...
  // ✈️ ТУРИЗМ
  // ======================
  const tourism = await prisma.category.create({
    data: {
      name: 'Туризм',
      slug: 'tourism',
      parentId: transportTourism.id,
  ...
  // ======================
  const travelGadgets = await prisma.category.create({
    data: {
      name: 'Гаджеты для путешествий',
      slug: 'travel-gadgets',
      parentId: transportTourism.id,
    },
  });
  ...
  await prisma.category.createMany({
    data: [
      { name: 'Дроны', slug: 'drones', parentId: travelGadgets.id },
      {
        name: 'Экшн-камеры',
        slug: 'action-cameras',
        parentId: travelGadgets.id,
      },
      {
      ...
      {
        name: 'Селфи палки / Триподы',
        slug: 'selfie-sticks-tripods',
        parentId: travelGadgets.id,
      },
    ],
  });
  ...
  // 🚗 АВТОТОВАРЫ
  // ======================
  const autoGadgets = await prisma.category.create({
    data: {
      name: 'Автогаджеты',
      slug: 'auto-gadgets',
      parentId: transportTourism.id,
  ...
  await prisma.category.createMany({
    data: [
      ...
      },
      {
        ...
        slug: 'child-car-seats',
        parentId: autoGadgets.id,
      },
      { name: 'FM Модулятор', slug: 'fm-modulators', parentId: autoGadgets.id },
      {
      ...
      {
        name: 'Автокомпрессоры',
        slug: 'car-compressors',
        parentId: autoGadgets.id,
      },
      {
        name: 'Автомагнитолы',
  ...
  // 🧰 АКСЕССУАРЫ ДЛЯ АВТО
  // ======================
  const autoAccessories = await prisma.category.create({
    data: {
      name: 'Аксессуары для салона автомобиля',
  ...
  await prisma.category.createMany({
    data: [
      {
        name: 'Наборы отвёрток',
        slug: 'toolkits',
        parentId: autoAccessories.id,
      },
      {
        name: 'Ароматизаторы для авто',
        slug: 'car-air-fresheners',
        parentId: autoAccessories.id,
      },
      {
        name: 'Защитный комплект',
        slug: 'protective-kits',
        parentId: autoAccessories.id,
      },
    ],
  });
  ...
  // 🔌 ОБЩИЕ АКСЕССУАРЫ
  // ======================
  const accessoriesTourism = await prisma.category.create({
    data: {
      name: 'Аксессуары для туризма',
      slug: 'accessories-tourism',
      parentId: transportTourism.id,
    },
  });
    ...
    data: [
      {
        ...
        parentId: accessoriesTourism.id,
      },
      {
        name: 'Туристические и походные',
